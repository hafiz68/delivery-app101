  ...
  try {
    ...
    if (resp.user && resp.user.active == false)
      return res
        ...
        const validation = Validation.createUserSchema.validate(req.body);
...
const updateUser = async (req, res) => {
  let { id } = req.params;
  const { userName, userEmail, userPassword, role, phoneNo, birthDate, address } = req.body;
  ...
  try {
    ...
    const resp4 = await userService.updateUser({ userName, phoneNo, birthDate, address }, id);
    if (resp4.error)
